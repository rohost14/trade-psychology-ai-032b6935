/**
 * DESIGN LAB — working copy of My Rules. Route: /my-rules-lab
 */
...
export default function MyRulesLab() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 pb-12">
      ...
      {/* Today's progress against those limits */}
      <div className="tm-card overflow-hidden">
        <div className="px-5 py-3.5 border-b border-border flex items-center justify-between">
          <h2 className="text-[14px] font-semibold text-foreground">Today so far</h2>
          {acceptedAt && (
            <span className="text-[11px] text-muted-foreground">
              Accepted {new Date(acceptedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
            </span>
          )}
        </div>
        <div className="p-5 space-y-4">
                  ...
                  <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                    <div
                      className={cn(
                        ...
                        // Green would read as "good" while counting toward a
                        // limit you do not want to reach. Neutral until it is
                        // worth noticing.
                        pct >= 100 ? 'bg-tm-loss'
                          : pct >= 75 ? 'bg-tm-obs'
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            {cooldownStatus?.limit_min == null ? (
              <span className="text-[14px] text-muted-foreground">not triggered today</span>
            ) : cooldownStatus.active ? (
              <span className="text-sm font-medium text-tm-obs">
