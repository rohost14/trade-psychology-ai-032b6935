...
**Reviewed: 23 of the 27, as of 30 Aug 2026 — see the REVIEW STATUS table
...
| 6 | `profit_giveaway` | **RETIRED — DELETED** | 27 Aug 2026 | 27 Aug 2026 | **MODIFY.** Subject is real — 26% of eligible sessions give back >=50% of peak, a genuine minority unlike Pattern 5, and nothing else sees the session high-water mark. But `min_peak` gates the green-to-red branch on the wrong quantity: **23 sessions worth -Rs 66,212 silenced** against -Rs 29,751 admitted. `erosion_pct` is unbounded (max 4.87) and is both severity key and re-arm metric. 1.9 alerts/day, severity oscillates, half the alert-days close green. Tiers 1.1 SE vs a 1.4 floor. Replay **48 alerts / 20 days** (the old queue note said 26/12 — wrong). `06-profit_giveaway/` |
...
| 16 | `excess_exposure` (§10) | ~~**DEFERRED — not reviewed**~~ **SUPERSEDED — see the RETIRED row below** | — | 1 Sep 2026 | Historical. The deferral read: blocked not on "capital not stateable" but on F17 rerouting its capital input through the canonical risk layer, which abstains on futures and short options until broker margin exists, so every number behind it was a fixture. **That deferral was lifted from a different direction** — the exposure-hierarchy work found the detector unsound on grounds that hold at any margin number, so it was retired rather than measured |
...
| 20 | `options_premium_avg_down` (§16) | **RETIRED — DELETED** | 30 Aug 2026 | 30 Aug 2026 | The §H0 note said it sat in no consolidation family. Measuring that first was right: it overlaps `same_symbol_obsession` on **70%** and `revenge_trade` on 48%. But the deeper finding is that **it was never an average-down** — 0 of 44 firings involved an open position, and its copy described `adding_to_adverse_position`, which already covers option premium averaging on **100% of its 64 firings**. Nothing to consolidate. 9 firings were a CE after a PE loss; 5 used a loss that had not yet happened. `20-options_premium_avg_down/` |
...
| 27 | `strategy_breakdown` (§27) | **RETIRED — DELETED** | 1 Sep 2026 | 2 Sep 2026 | **RETIRED / COMPLETE** — deferred 1 Sep, retired **2 Sep 2026** once the deferral's own unblock condition was examined rather than waited on. It required a win-rate collapse **AND** a profit-factor collapse together; the profit-factor half **never bound** — 4 firings, the **identical set** to `win_rate_collapse`, **zero unique**. PF collapse is not rare alone (6 of 26 qualifying sessions) but as the second half of an `AND` it excluded nothing, because a session winning 11% of its trades almost always has a wrecked profit factor: the two conditions are **not independent** in the way the design assumed. **Not retired for firing rarely — retired for being a second name for one finding.** `win_rate_collapse` keeps the subject and both baselines and was deliberately **not** made louder; the `performance` domain still cannot reach a trader, which is an **open product question** a retirement may not answer. `25-27-performance-trio/` |
| 28 | `overexposure` / `portfolio_concentration` / `holding_loser` (§A2-A4) | **ALL THREE RETIRED — CLOSED** | 1 Sep 2026 | 2 Sep 2026 | **`holding_loser` RETIRED 2 Sep**: a snapshot plus a stopwatch; the hold-duration substitute failed the persistence test (0.62 first half vs 2.54 second, intraday 1.04 at shuffle p = 0.343, median 0.98). Not replaced — no MTM capture, and the comparison was not promoted to analytics because it fails the same test. **`overexposure` RETIRED 2 Sep as a NAME ONLY**: the alias was already dead, `_overexposure_task` emits `constitution_violation`/`max_trade_risk`, gates on the declared limit and abstains without a capital requirement — the entry-time check is untouched. Original 1 Sep review text follows: | Separate pipeline (`position_monitor_tasks`), reviewed as a group — which is what made the finding visible. **`portfolio_concentration` measured how FEW positions were open**: with n positions the top underlying's share is at least 1/n, so a two-position book had a **50% floor against a 40% cut** and it fired **206 of 206**, 69% of all firings, the rate falling monotonically as the book diversified. **RETIRED.** `overexposure`'s quantity is wrong on arithmetic (100% of futures), not on the price substitution — **MODIFY, blocked on the margin decision**. `holding_loser`: duration is measurable, the predicate is not — **RESEARCH FURTHER**. Neither firm finding depends on the fixture prices. `28-position-monitor/` |
| 16 | `excess_exposure` (§10) | **RETIRED — DELETED** | 1 Sep 2026 | 1 Sep 2026 | Shipped with the exposure hierarchy (`0602aa8`), which is why its earlier DEFERRED row is superseded. **No universal exposure threshold survives and none replaced its 5/10.** A trader who DECLARED 40% was told DANGER at 35% — inside their own rule — because `safety_bounds` clamps a declared value so it may only tighten, and the alert could not tell 35% from 45%. Outcome evidence never supported it (per round 0-5% won 40.2%, 5-10% 37.4%, 10-15% 43.1%, 15-25% 43.9%; only 25%+ separated, at n=10 with 81% of that bucket from ONE position). Single-position exposure is now solely a breach of the trader's own declared limit via `constitution_violation`'s `max_trade_risk`; the entry-time arm emits that same pattern type and rule so `_pattern_dedup_key` collapses entry and exit. **Also shipped: `sl_percent_options`/`sl_percent_futures` no longer invent 50.0/1.0 as `Source.FACT`.** `28-position-monitor/` |
| M | `death_spiral` (§A1, meta) | **RETIRED — DELETED** | 2 Sep 2026 | 2 Sep 2026 | Reviewed last and correctly so — it consumed the others' output, so every earlier retirement moved its count as arithmetic. **A summary of alerts already delivered, not a state.** Without rules **10 / 203**, all danger, **set-identical** to "a danger emotional + a danger risk alert today" and a strict SUBSET of the simpler "2 danger detectors" rule, whose four excluded sessions include the 4th-worst day in the book (−₹11,015). With one declared rule **79 / 203 (38.9%)**, `constitution_violation` in **100%**, and in **61%** both "independent" domains came from two detectors reading the SAME `daily_loss_limit`. **69%** followed an already-delivered danger alert; **15%** incremental. Its **absorption branch was DEAD CODE** — 0 `absorbed:` rows ever — and the master doc asserted the opposite for months. Replay **87 → 0**, every other detector identical, P&L byte-identical. Historical rows KEPT and marked Retired in the UI. `A1-death_spiral/` |
| H | `capital_mismatch` (§A5) | **EXCLUDED BY DECISION — not a review** | 1 Sep 2026 | 1 Sep 2026 | Traced end to end during 28's scope investigation: **it never reads a position, a trade, an order, a fill or any P&L.** Producer `maintenance_tasks.check_capital_reality` on a real beat; compares declared `UserProfile.trading_capital` against the latest `MarginSnapshot`. It is a **precondition for the exposure detectors' correctness** — both divide by `trading_capital` — which is a dependency, not an overlap, so reviewing it here would mix "is this behaviour worth alerting on" with "is this settings field stale". §H0 F21 confirmed its absence from `_ALIAS_NATURE` is correct and `test_f21_capital_mismatch_is_excluded_from_death_spiral_on_purpose` pins it. **Routed to the consolidated pending pass as housekeeping**, with the dependency recorded. `28-position-monitor/scope_investigation.md` |
