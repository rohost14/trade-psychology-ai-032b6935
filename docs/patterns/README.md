...
|---|---|---|---|
| [00](00-shared/) | *shared* — the pattern baseline (33 types at the time of writing; **27 since Patterns 4, 6, 9, 10, 11, 14 and 15 were retired**), the martingale/adding distinction, the replay close-out | — | **578 alerts / 203 sessions** |
...
| [17](17-session_meltdown/) | `session_meltdown` | **REVIEWED — MODIFY, awaiting approval.** Source-list #11, and the **loudest detector reviewed so far**: `notification_level=4`, starts a soft cooldown, drives the danger zone. The March note — *invents a limit at 5% of capital* — is **already addressed**: it carries `limit_is_declared` and never calls a derived number "yours". But the **5% is still hardcoded inline** while `account_risk.py`, written for this detector and already in the context, goes unread; `meltdown_caution_pct`/`meltdown_danger_pct` have **no THRESHOLD_SPECS record**; and the copy claims *"pace of trading"*, which it does not measure. Firing is wholly limit-driven — **226 events / 52% of sessions** at a ₹50k account, **1** at ₹5L | **111 / 53** at a ₹5,000 limit |
| [18](18-early_exit/) | `early_exit` | **REVIEWED — MODIFY, awaiting a product decision.** Source-list #14. The **concept is right** — winner-hold vs loser-hold is the standard disposition-effect measure and the only observable answer to "was that exit early". What fails is the **scope**: at 3-5 trades per side the ratio is **indistinguishable from chance, shuffle null p = 0.610**, and no threshold value fixes that. The effect is also **absent in this book** — winners held 41.0min vs losers 36.7min, ratio 1.12. Meanwhile `baseline_service` already computes `avg_winner_hold_min`/`avg_loser_hold_min` over **276 and 413 trades** and **nothing reads them**. Also: `trigger=session` declared but not honoured, and `early_exit_winner_max_min` declares a metric (`winner_hold_p50`) that is **never produced**, so a PERSONAL_BASELINE is permanently global | **3 / 2** |
