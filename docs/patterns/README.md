...
|---|---|---|---|
| [00](00-shared/) | *shared* — the pattern baseline (33 types at the time of writing; **23 since Patterns 4, 6, 9, 10, 11, 14, 15, 18, 19, 20 and 21 were retired**), the martingale/adding distinction, the replay close-out | — | **578 alerts / 203 sessions** |
...
| 16 | `excess_exposure` | **DEFERRED — not reviewed.** Source-list #10. F17 rerouted its capital input through the canonical risk layer, which **abstains on futures and short options** until broker margin exists — and live margin capture has never run, so every number behind it is a fixture. Reviewing now would judge a temporarily silent detector. **Blocked on live broker-margin validation.** Its original audit note — *zero test mentions AND excluded from replay* — is a separate open problem | not measured |
