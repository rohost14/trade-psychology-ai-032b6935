...
| [15](15-cooldown_violation/) | `cooldown_violation` | **REVIEWED — DEFER, awaiting one product answer.** Source-list #8. **Its precondition never occurs on the live path**: cooldown rows are written only by `trigger_intervention`, reachable from `POST /sync/all` and one explicit endpoint, and **no Celery task calls it**. The behaviour it names is **completely covered** by `constitution_violation`'s cooldown rule, which uses the trader's OWN declared value at **danger** — measured 181 events against this detector's **0**. Its registry copy describes that other detector's mechanism, not its own. DELETE is the likely destination but needs one answer: will danger-zone interventions ever run from the live pipeline? F18's entry-time fix is correct and kept | **0** (no cooldown rows exist in a tradebook) |
