...
- **`intent_tasks` re-learn (18:15 IST):** loops **every** active account, calls `ai_personalization_service.learn_patterns(days_back=90)` per account, in **one task**, on **one worker**, sharing **one DB session**. At 10k accounts → runs for hours, blocks a worker, holds a long transaction/connection. **CORRECTION (deep-review P5-C2, grep-verified):** `learn_patterns` is **pure SQL+Python stats — NO LLM call** (no OpenRouter/ai_service refs anywhere in `ai_personalization_service`). The earlier "10k LLM calls" claim here was wrong. The sequential all-account loop is still the real bottleneck; there is **no LLM cost/rate-limit explosion** from re-learn. *(Also: this task is currently orphaned by the P0 queue-routing gap — deep-review P0-F1 — so it doesn't run at all today.)*
