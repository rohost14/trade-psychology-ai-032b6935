...
> ✅ **FIXED 2026-07-26** — added `_check_csrf_origin` to `get_current_admin`: any non-safe method (POST/PUT/DELETE/PATCH) must carry a browser `Origin` in the CORS allow-list (exact list + regex, mirrored so nothing that passes CORS breaks). Forged cross-site writes (e.g. `evil.com`) → 403; safe methods + no-Origin (non-browser) untouched. Verified all cases. Admin FE is cookie-only (`credentials: include`) so browsers always send Origin on its cross-origin writes — no FE change needed.
> ⚠️ **Note:** this is **Origin-based** CSRF defense (defense-in-depth). A double-submit CSRF token is the stronger belt-and-suspenders follow-up (needs a FE change). Also independent of P0-F8 (tighten the CORS private-IP regex separately).
...
## ⚪ P3
- **A5** ✅ **FIXED 2026-07-26** — null/blank Zerodha email no longer matched (`if zerodha_email:` guard before the `User.email ==` lookup) → no mis-merge; falls back to creating a fresh user (BrokerAccount still keys on broker_user_id). Original: OAuth identity is keyed on the Zerodha profile **email** (`User.email == zerodha_email`). If `zerodha_email` is ever null/absent, `User.email == None` can mis-match, and two Zerodha logins sharing an email would merge into one User. Add a not-null guard + prefer `broker_user_id` as the stable key. (BrokerAccount already keys on `broker_user_id` — good; the User row is the soft spot.)
