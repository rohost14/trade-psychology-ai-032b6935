...
> ✅ **RESOLVED 2026-07-26 (code):** the worker now uses a **NullPool** async engine (gated on `CELERY_WORKER=1`, set in the Procfile worker+beat commands), so each `asyncio.run()` task opens+closes its own connection — no cross-loop reuse, no `pool_timeout` starvation under burst. The web process keeps its real pool. Verified pool selection by flag. **Load test (Gate 4) now only tunes/confirms** (worker count, DB connection budget) — the correctness fix is in.
