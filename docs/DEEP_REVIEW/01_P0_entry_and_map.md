...
> ✅ **PENDING PART NOW DONE 2026-07-26:** `setup_logging()` is wired into a Celery `worker_process_init` signal (`celery_app.py`), so each worker child gets JSON logging + the Redis error-feed handler → **task errors now reach the admin error-feed**. (Accepted tradeoff: the error-feed handler does a best-effort blocking Redis write per ERROR log in the worker.)
