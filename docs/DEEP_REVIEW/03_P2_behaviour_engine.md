...
> ✅ **FIXED 2026-07-26 (dual engine retired, archived not deleted)** — new `services/behavior_summary.py` builds `patterns_detected`/`behavior_score`/`emotional_tax` from the live engine's stored **RiskAlerts** (+ session risk score + factual flagged-trade P&L). Repointed both consumers (`api/behavioral.py` `/analysis`+`/patterns`; `analytics.py` behavior-tab); dropped the dead `/trade-tags` route; **`git mv behavioral_analysis_service.py → services/_archive/`**. Verified: no live importers remain, app boots, pure mapper tested (`test_behavior_summary.py`).
> ⚠️ **Gate-3 note:** runtime FE output (export-report text, AI narrative `behavior_score`/`emotional_tax` numbers) changed to the real-engine values — visually validate in the live/staging test (can't render the FE here). `behavior_score` is now the session risk score (0-100, higher=worse); `emotional_tax` is factual flagged-trade realized P&L.
...
> **CORRECTION (P5-C1, verified):** the trigger is **CompletedTrade-rebuild** events (tradebook import, manual `/recalculate-pnl` [no caller], late-fill replay) — **NOT** a nightly/EOD batch (nothing in eod_sync/webhook calls `calculate_and_update_pnl`). Pure-live users keep intact links. **Severity downgrades P1→P2** (latent/event-triggered). Fix unchanged (`_stable_ct_id` in the live builder).

### E2 · Alert→trade linkage is severed on CompletedTrade rebuild (elevation of P1-M6) · correctness/data-integrity
> ✅ **FIXED 2026-07-26 (via M6)** — the live builder now uses the shared deterministic `stable_completed_trade_id`, so a rebuild reuses the same id and `trigger_completed_trade_id` is no longer NULLed. Proven by the ledger==batch id-agreement test.
