...
> ⚠️ **17 high remain = dev/build tooling** (esbuild/vite, eslint chain, next/geist, sharp). **`npm audit fix --force` attempted 2026-07-26 and REVERTED:** it dropped vulns 21→2 and passed build/typecheck/lint/test, BUT it force-jumped **vite 5→8** (to rolldown) and **eslint 9→10** with **invalid peer deps** (`@vitejs/plugin-react-swc` doesn't support vite 8; `eslint-plugin-react-hooks` doesn't support eslint 10) — too fragile/bleeding-edge for a trading app. **Low real risk:** these are **build-time** tools, not shipped in the production bundle. **Correct fix = a deliberate incremental upgrade** (vite 5→6→7 with a compatible plugin-react-swc, eslint stays ≤9), not `--force`. CI `npm audit` stays non-blocking until then.
