...
| # | Finding | Phase |
|---|---|---|
| R1 | ✅ **FIXED 2026-07-26** — worker → `prefork` + **NullPool async engine in the worker** (gated on `CELERY_WORKER=1`), so per-task `asyncio.run` loops don't reuse pooled asyncpg connections; web keeps its pool. D18 resolved. Load test (Gate 4) now only tunes. | P3 |
| F2 | ✅ **FIXED 2026-07-26** — `setup_logging()` wired at web startup AND in the Celery `worker_process_init` signal → JSON logs + admin error-feed + request-id filter active in BOTH processes (also fixes F11). | P0 |
...
| F4 | ✅ **FIXED 2026-07-26 (limiters)** — both rate limiters now use **async Redis** (await), no longer blocking the event loop. (The error-feed logging handler stays sync — `logging.Handler.emit` can't await; only fires on errors.) | P0 |
...
| M3 | ✅ **FIXED 2026-07-26 (test-first)** — `get_unrealized_pnl` applies the lot multiplier (pure helper + `test_pnl_multiplier.py`). | P1 |
| M5 | ✅ **FIXED 2026-07-26** — batch FIFO now resolves the MCX multiplier via the ledger's fallback (Zerodha `Position.multiplier`) instead of silently using 1 for untabulated contracts. | P1 |
