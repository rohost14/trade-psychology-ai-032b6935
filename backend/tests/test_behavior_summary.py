...
(patterns_detected / emotional_tax) must come from the REAL engine's stored
RiskAlerts, not the legacy behavioral_analysis_service. Pure mapper test (no DB).

`behavior_score` was dropped from this shape 2026-08-13 with the session risk
score itself — docs/GLOBALS_DERIVATION.md. No caller ever read it.
"""
...
def test_empty():
    s = summarize_behavior([], flagged_pnl=0.0)
...
def test_distinct_patterns_worst_severity_wins():
    ...
    s = summarize_behavior(alerts, flagged_pnl=-1234.5)
...
def test_focus_area_is_worst_pattern():
    ...
    s = summarize_behavior(alerts, flagged_pnl=0)
...
def test_shape_has_keys_the_fe_reads():
    s = summarize_behavior([_a("fomo_entry", "caution", "x")], -10)
    for k in ("patterns_detected", "emotional_tax", "top_strength", "focus_area"):
        assert k in s
