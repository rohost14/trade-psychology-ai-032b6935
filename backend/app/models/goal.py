...
class Goal(Base):
    ...
    broker_account_id = Column(UUID(as_uuid=True), ForeignKey("broker_accounts.id", ondelete="CASCADE"), nullable=False, unique=True)
    ...
    # Primary trading segment (EQUITY, FNO, COMMODITY, CURRENCY)
    primary_segment = Column(String(20), default="EQUITY")

    # Trading hours - segment-aware defaults
    # EQUITY/FNO: 09:15-15:30, COMMODITY: 09:00-23:30, CURRENCY: 09:00-17:00
    allowed_trading_start = Column(String(10), default="09:15")
    allowed_trading_end = Column(String(10), default="15:30")

    # Segment-specific hours (JSON: {"EQUITY": {"start": "09:15", "end": "15:30"}, ...})
    segment_hours = Column(JSONB, default=dict)
